...
export default {
    ...
    query: async (req,res,next) =>{
        try{
            const reg =await models.proyecto.findOne({_id:req.query._id})
    ...
    list: async (req,res,next) =>{
        try{
            ...
            const reg=await models.proyecto.find({$or:[{'nombre':new RegExp(valor,'i')},{'descripcion':new RegExp(valor,'i')}]},{createAt:0})
            .populate('empresa',{nombre:1})
            .sort({'createdAt':-1});
    ...
    update: async (req,res,next) =>{
        try{
            const reg =await models.proyecto.findByIdAndUpdate({_id:req.body._id},{empresa:req.body,nombre:req.body.nombre});
            res.status(200).json(reg);
        } catch (e){
            res.status(200).send({
                message:'Error'
