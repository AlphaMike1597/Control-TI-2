...
export default {
    ...
    list: async (req,res,next) =>{
        try{
            const reg=await models.equipo.find({$or:[{'nombre':new RegExp(valor,'i')},{'descripcion':new RegExp(valor,'i')}]},{createAt:0})
            .sort({'createdAt':-1});
    ...
    update: async (req,res,next) =>{
        try{
            const reg =await models.equipo.findByIdAndUpdate({_id:req.body._id},{nombre:req.body.nombre});
            res.status(200).json(reg);
        } catch (e){
            res.status(200).send({
                message:'Error'
    ...
    remove: async (req,res,next) =>{
        try{
            const reg = await models.equipo.findByIdAndDelete({_id:req.body._id});
            res.status(200).json(reg);
        } catch (e){
            res.status(200).send({
                message:'Error'
